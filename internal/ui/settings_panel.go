	...
	if len(selectOptions) > 0 {
		components = append(components,
			...
			discord.NewActionRow(
				discord.NewStringSelectMenu(settingsModuleID+":select", "モジュールを選択...", selectOptions...),
			),
		)
	} else {
		components = append(components,
...
func BuildModulePanel(info module.Info, enabled bool, settingsPanel []discord.LayoutComponent) discord.MessageUpdate {
	...
	components = append(components,
		discord.NewLargeSeparator(),
		discord.NewActionRow(
			discord.NewButton(toggleStyle, toggleLabel, fmt.Sprintf("%s:toggle:%s", settingsModuleID, info.ID), "", 0),
			discord.NewSecondaryButton("← 戻る", settingsModuleID+":back"),
		),
	)
